...
def search_cod(request):
    ...
    result = Formulari.objects.filter(cod__icontains=string)
    vals = result.values_list('cod')
    for val in vals:
        for i in val:
            res.append(i)
...
def formulari_list(request):
    formulari = Formulari.objects.filter(
        ~Q(stato='CH'), ~Q(stato='IA')).order_by('-data')
    return render(request, 'formulari/formulari_list.html', {'formularis': formulari})
...
def anagrafica_list(request):
    array = Anagrafica.objects.annotate(doc_ap=Subquery(Anagrafica.objects.filter(pk=OuterRef('pk')).filter(
        formulari__stato="AP").annotate(doc_ap=Count('formulari')).values('doc_ap'), output_field=models.IntegerField()))
...
def anagrafica_riepilogo(request, pk):
    fqs = Formulari.objects.filter(comm=pk).filter(
        stato='AP').order_by('-data')
    gks = []
    fps = []
    gsa = []
    for f1 in fqs:
        ...
        if f1 and f1.ripa not in gks:
            gks.append(f1.ripa)
    for g1 in gks:
        gs = Ripartizioni.objects.filter(formulari__pk__in=fps).filter(
            pk=g1.pk).annotate(totale=Sum('formulari__imp')).values('nome', 'totale', 'pk')
        appo = dict(gs[0])
        gsa.append(appo)
    context = {'fqss': fqs}
    context['gsas'] = gsa
...
def prezzi_ultimi(request):
    ...
    for q1 in qs:
        for m1 in ms:
            valk = Prezzi.objects.filter(comm__nome=q1).filter(
                mat__mat=m1.mat).order_by('-data').values('id')[:1]
            if len(valk):
                appo = dict(valk[0])
...
def riepiloghi_list(request):
    array = Riepiloghi.objects.annotate(
        totale=Sum('formulari__imp')).order_by('-data')
    return render(request, 'formulari/riepiloghi_list.html', {'arrays': array})


@login_required(login_url='/login/')
def riepiloghi_f_anagrafica(request, pk):
    array = Riepiloghi.objects.filter(comm__pk=pk).annotate(
        totale=Sum('formulari__imp')).order_by('-data')
    return render(request, 'formulari/riepiloghi_list.html', {'arrays': array})
...
def riepiloghi_generate(request, pk):
    nr = Riepiloghi(comm=Anagrafica.objects.get(
        pk=pk), data=datetime.date.today(), autore=request.user, stato='PR')
    nr.save()
